...
```mermaid
flowchart TD
  START([Usuario Migrante]) --> A1[1. Ingreso Portal SNM]
  A1 --> A2[2. Carga de Documentos]
  A2 --> A3[3. Validación Preliminar]
  
  A3 --> A3A{Documento Válido?}
  A3A -->|No| A3B[Solicitar Nueva Carga]
  A3B --> A2
  A3A -->|Sí| A4[4. Preprocesamiento de Imagen]
  
  A4 --> A5[5. Extracción OCR - Amazon Textract]
  A5 --> A6[6. Validación Automática de Campos]
  
  A6 --> A6A{Campos Críticos Válidos?}
  A6A -->|No| A6B[Retroalimentación al Usuario]
  A6B --> A2
  A6A -->|Sí| A7[7. Almacenamiento Seguro]
  
  A7 --> A8[8. Registro de Trazabilidad]
